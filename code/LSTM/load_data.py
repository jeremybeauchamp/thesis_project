...
# This function formats the data into the form it needs to be for the models.
#
# Parameters:
# 	filename = The filename of the pickle file
#
# 	pretrain = True if you are generating the pretraining dataset, false if you are
# 	generating the data for individual subjects.
#
# 	patient = the id of the subject to load. If pretrain is True this should be None.
#
# 	unrestricted = True for unrestricted False for inertial.
#
# 	combined = True if the validation data should be added to the training data.
# 	False otherwise.


def load_data(
    filename, pretrain=False, patient=None, unrestricted=True, combined=False
):
    fd = open(filename, "rb")
    if pretrain:
