...
def load_from_xml(
    fileNamesAndIDs, res=5, includeCats=[], dtFormat="%d-%m-%Y %H:%M:%S", verbose=True
):
    data = {}
    timeStamps = {}
    isRealBGL = {}
    ...
    if not isinstance(includeCats, list):
        raise Exception(
            "The category list to include must be a LIST of category labels."
        )
    ...
    for pID, fname in fileNamesAndIDs:
        ...
        if "timeOfDay_real" in includeCats:
            ...
            timeOfDay_real = {
                ts: (ts - ts.replace(minute=0, hour=0)).total_seconds()
        ...
        if "timeOfDay_ticks" in includeCats:
            print(
                " > Time of day is being included as one feature with ticks on the hour..."
            )
            timeOfDay_ticks = {}
            prevHour = -1
            for ts in timeStamps[pID]:
        ...
        while item.tag:
            try:
                if item.tag == "finger_stick" and "finger_stick" in includeCats:
                    ...
                    item = next(xmlIter)
                    while item.tag == "event":
                        DT = dt.datetime.strptime(item.attrib["ts"], dtFormat).replace(
                            second=0
                        )
                    ...
                    while item.tag == "event":
                        beginDT = dt.datetime.strptime(
                            item.attrib["ts_begin"], dtFormat
                        ).replace(second=0)
                        endDT = dt.datetime.strptime(
                            item.attrib["ts_end"], dtFormat
                        ).replace(second=0)
                        while beginDT <= endDT:
                    ...
                    while item.tag == "event":
                        ...
                        while beginDT <= endDT:
                            ...
                            if 'bwz_carb_input' in item.attrib:
                                bolus_carbs[beginDT] = float(item.attrib["bwz_carb_input"])
                            else:
                                bolus_carbs[beginDT] = 0.0
                    ...
                    while item.tag == "event":
                        ...
                        if endDT - beginDT > dt.timedelta(hours=12) and verbose:
                            print(
                                " Warning: long sleep detected from {} to {}, duration={} hr".format(
                                    beginDT,
                                    endDT,
                                    (endDT - beginDT).total_seconds() / 3600,
                                )
                            )
                        while beginDT <= endDT:
                            sleep[beginDT] = 1
                    ...
                    while item.tag == "event":
                        ...
                        if endDT - beginDT > dt.timedelta(hours=12) and verbose:
                            print(
                                " Warning: long work detected from {} to {}, duration={} hr".format(
                                    beginDT,
                                    endDT,
                                    (endDT - beginDT).total_seconds() / 3600,
                                )
                    ...
                    while item.tag == "event":
                        ...
                        if DT in meal and verbose:
                            print(
                                " Warning: hypo action already in 'meals' at {} with carbs={}, replacing with {}...".format(
                                    DT, meal[DT], item.attrib["carbs"]
                                )
                            )
                    ...
                    item.tag == "basis_heart_rate" and "basis_heart_rate" in includeCats
                ):
                    ...
                    item.tag == "basis_skin_temperature"
                    and "basis_skin_temperature" in includeCats
                ):
                    ...
                    item.tag == "basis_air_temperature"
                    and "basis_air_temperature" in includeCats
                ):
...
def calculate_stats(
    ...
    for d in ["training", "testing", "validation"]:
        dataRaw, timeStamps, isRealBGL = load_from_xml(
            zip(patients, files[d]),
            time_res,
            ["bgl", "meal", "bolus", "basal"],
            verbose=False,
        )

        dataRaw = pre_process_data(
            patients,
            dataRaw,
        ...
        if d == "training":
            for p in patients:
                ...
                meal_scaling[p] = {
                    "min": mins[p][index["meal"]],
                    "max": maxs[p][index["meal"]],
                }
                bgl_scaling[p] = {
                    "min": mins[p][index["bgl"]],
                    "max": maxs[p][index["bgl"]],
                }
                insulin_scaling[p] = {
                    "min": mins[p][index["bolus"]],
                    "max": maxs[p][index["bolus"]],
                }
            ...
            if combined:
                validation_data, timeStampsValidation, _ = load_from_xml(
                    zip(patients, files["validation"]),
                    time_res,
                    ["bgl", "meal", "bolus", "basal"],
                    verbose=False,
                )
            ...
            for p in patients:
                if combined:
                    data_for_averages = np.append(
                        dataRaw[p], validation_data[p], axis=0
                    )
                    timestamps_for_averages = np.append(
                        timeStamps[p], timeStampsValidation[p]
                    )
                else:
    ...
    information = {
        ...
        "scaling": {
            "bgl": bgl_scaling,
            "meal": meal_scaling,
            "insulin": insulin_scaling,
        },
        "pretrain_scaling": {
            "bgl": pretrain_bgl_scaling,
            "meal": pretrain_meal_scaling,
            "insulin": pretrain_insulin_scaling,
        },
        "maxs": maxs,
        "mins": mins,
...
def pre_process_data(
    patients, raw_data, meal_bolus_distance=10, time_res=5, use_bolus_carbs=False
):
    ...
    for p in patients:
        for i in range(len(raw_data[p])):
            ...
            if meal_bolus_distance != None:
                if (
                    raw_data[p][i][index["bolus"]] != 0
                    and raw_data[p][i][index["bolus_carbs"]] != 0
                    and (
                        raw_data[p][i][index["bolus_type"]]
                        == bolus_type_values["normal"]
                        or raw_data[p][i][index["bolus_type"]]
                        == bolus_type_values["normal dual"]
                    )
                ):
                    ...
                    while not closest_meal_found:
                        if left_index < 0 or right_index >= len(raw_data[p]):
                            ...
                            if (
                                raw_data[p][left_index][index["meal"]] != 0
                                and raw_data[p][right_index][index["meal"]] != 0
                            ):
...
def create_examples(
    ...
    for j in possible_times:
        ...
        if combo:
            ...
            pretrain_secondary_event = scaled_data[event_start + distance][
                index["meal"]
            ]
        ...
        if not ignore:
            if combo:
                example = {
                    ...
                    "input3": np.array(
                        [bgl_target]
                        + [time_value / int(max_time_value * time_res)]
                        + [(avg - scaling["min"]) / scaling["max"]]
                        + [secondary_event]
                    ),
                    "case": case,
                }
            ...
            if combo:
                pretrain_example = {
                    "label": pretrain_event,
                    "input1": pretrain_seq1,
                    "input2": pretrain_seq2,
                    "input3": np.array(
                        [pretrain_bgl_target]
                        + [time_value / int(max_time_value * time_res)]
                        + [(avg - pretrain_scaling["min"]) / pretrain_scaling["max"]]
                        + [pretrain_secondary_event]
                    ),
                    "case": case,
                }
            else:
...
def create_vectors(patients, all_examples):
    ...
    for d in ["training", "testing", "validation"]:
        ...
        for c in ["1", "2", "3"]:
            ...
            for l in ["input1_layer", "input2_layer", "input3_layer"]:
                pretrain_feature_vectors[d][c][l] = np.array(
                    pretrain_feature_vectors[d][c][l]
                )
...
def load_carbs_data(
    ...
    if stats_file == None:
        information = calculate_stats(
            patients, files, event_type="meal", time_res=time_res, combined=combined
        )
    else:
    ...
    for d in ["training", "testing", "validation"]:
        raw_data, time_stamps, bgl_mask = load_from_xml(
            zip(patients, files[d]),
            time_res,
            ["bgl", "meal", "bolus", "basal"],
            verbose=False,
        ...
        for p in patients:
            ...
            for i in range(len(scaled_data[p])):
                ...
                if (
                    scaled_data[p][i][index["meal"]] != 0
                    and scaled_data[p][i - 2][index["bolus"]] == 0
                ):
                    event_start = i
...
def load_bolus_data(
    ...
    if stats_file == None:
        information = calculate_stats(
            patients, files, event_type="bolus", time_res=time_res, combined=combined
        )
    else:
        fd = open(stats_file, "rb")
        information = pickle.load(fd)
    ...
    for d in ["training", "testing", "validation"]:
        ...
        for p in patients:
            ...
            for i in range(len(scaled_data[p])):
                if scaled_data[p][i][index["bolus"]] != 0 and (
                    scaled_data[p][i][index["bolus_type"]]
                    == bolus_type_values["normal"]
                    or scaled_data[p][i][index["bolus_type"]]
                    == bolus_type_values["normal dual"]
                ):
                    event_start = i
                    examples = create_examples(
...
def load_combo_data(
    ...
    for d in ["training", "testing", "validation"]:
        ...
        for p in patients:
            ...
            for i in range(len(scaled_data[p])):
                if (
                    scaled_data[p][i][index["bolus"]] != 0
                    and scaled_data[p][i][index["bolus_carbs"]] != 0
                ) and (
                    scaled_data[p][i][index["bolus_type"]]
                    == bolus_type_values["normal"]
                    or scaled_data[p][i][index["bolus_type"]]
                    ...
                    if (
                        i + distance < len(scaled_data[p])
                        and scaled_data[p][i + distance][index["meal"]] != 0
                    ):
                        event_start = i
                        examples = create_examples(
                            scaled_data[p],
...
if __name__ == "__main__":
    ...
    training_files = [
        "/home/jeremy/EventPrediction/data/data/training/"
        + pID
        + "-ws-training-new.xml"
        for pID in patients
    ]
    ...
    validation_files = [
        "/home/jeremy/EventPrediction/data/data/validation/"
        + pID
        + "-ws-validation.xml"
        for pID in patients
    ]
    ...
    if carbs:
        carbs_data, carbs_info = load_carbs_data(
            patients,
            files,
    ...
    if bolus:
        bolus_data, bolus_info = load_bolus_data(
            patients,
            files,
    ...
    if combo:
        combo_data, combo_info = load_combo_data(
            patients,
            files,
            stats_file="stats_bolus.pkl",
            approach=2,
