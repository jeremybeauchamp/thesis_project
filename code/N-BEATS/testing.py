...
def evaluate(
    ...
    for patient in patient_list:
        ...
        testing_loader = DataLoader(dataset=dataset, shuffle=True, drop_last=False)

        validation_loader = DataLoader(
            dataset=validation, shuffle=True, drop_last=False
        )
        ...
        for run in range(runs):
            network = (
                torch.load("./patient_" + patient + "_" + str(run) + ".model")
                .get("model")
                .cuda()
            )
        ...
        for run in range(runs):
            ...
            for step, (x, x2, x3, y) in enumerate(testing_loader):
                ...
                fd.write(
                    str((prediction.cpu().data.numpy()[0][0] * scale_max) + scale_min)
                    + ","
                    + str((y.cpu().data.numpy()[0][0] * scale_max) + scale_min)
                    + "\n"
                )
                predictions = predictions + prediction.cpu().data.numpy().tolist()
...
def compute_final_results(results, best_models, patient_list):
    ...
    overall = {
        "AVG": {"RMSE": average_rmse, "MAE": average_mae},
