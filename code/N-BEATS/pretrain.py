    ...
    for run in range(runs):
        ...
        for e in range(num_epochs):
            ...
            for step, (x, x2, x3, y) in enumerate(training_loader):
                ...
                if unrestricted:
                    forecast, forecasts, backcasts, backsum, backcast_targets = network(
                        x.to(device), x2.to(device), x3=x3.to(device)
                    )
                else:
                    forecast, forecasts, backcasts, backsum, backcast_targets = network(
                        x.to(device), x2.to(device)
                    )
                ...
                with torch.no_grad():
                    ...
                    if unrestricted:
                        (
                            ...
                            backsum,
                            backcast_targets,
                        ) = network(vx.to(device), vx2.to(device), x3=vx3.to(device))
                    else:
                        (
                            forecast,
                            forecasts,
                            ...
                            backsum,
                            backcast_targets,
                        ) = network(vx.to(device), vx2.to(device))
