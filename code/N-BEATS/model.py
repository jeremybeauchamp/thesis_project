...
class Block(nn.Module):
    def __init__(
        ...
        self.input_vars = (
            input_vars  # Number of variables at each step of input sequence
        )
        self.ex_vars = ex_vars  # Number of variables appended to lstm output
        self.backcast_len = backcast_len
        self.forecast_len = forecast_len
        self.num_fc_layers = hyper_parameters[
            "fc_layers"
        ]  # Number of fully connected layers after the LSTM(s)
        self.fc_units = hyper_parameters[
            "fc_units"
        ...
        if self.unrestricted:
            if self.num_fc_layers == 1:
                self.fc_layers.append(
                    nn.Linear(
                        (self.r_units * 3) + self.ex_vars,
                        self.backcast_len + self.forecast_len,
                    )
                )
            else:
                self.fc_layers.append(
                    nn.Linear((self.r_units * 3) + self.ex_vars, self.fc_units)
                )
        else:
            if self.num_fc_layers == 1:
                self.fc_layers.append(
                    nn.Linear(
                        (self.r_units * 2) + self.ex_vars,
                        self.backcast_len + self.forecast_len,
                    )
                )
            else:
                self.fc_layers.append(
                    nn.Linear((self.r_units * 2) + self.ex_vars, self.fc_units)
                )
        ...
        if self.num_fc_layers >= 2:
            self.fc_layers.append(
                nn.Linear(self.fc_units, self.backcast_len + self.forecast_len)
            )
    ...
    def forward(self, x, x2, x3=None):
        ...
        if samples < self.bs:
            x = F.pad(
                input=x,
                pad=(0, 0, 0, 0, 0, self.bs - samples),
                mode="constant",
                value=0,
            )
            x2 = F.pad(
                input=x2, pad=(0, 0, 0, self.bs - samples), mode="constant", value=0
            )
            if x3 is not None:
                x3 = F.pad(
                    input=x3,
        ...
        if x3 is not None:
            ...
            h2_0 = (
                h2_0.reshape((self.bs, 1, self.r_units))
                .permute(1, 0, 2)
                .data.contiguous()
            )
            c2_0 = (
                c2_0.reshape((self.bs, 1, self.r_units))
                .permute(1, 0, 2)
                .data.contiguous()
            )
    ...
    def forward(self, backcast, x2, x3=None):
        ...
        for s in range(len(self.stacks)):
            (
                ...
                current_forecasts,
                current_backcasts,
                current_backcast_targets,
            ) = self.stacks[s](forecast, x2, backcast, backsum, x3=x3)
