...
def tune_lr(data_file,
	...
	learning_rates = [0.01, 0.001, 0.0002]
	...
	for lr in learning_rates:
		hyper_parameters['learning_rate'] = lr
		results[str(lr)] = run(data_file,
							   hyper_parameters,
							   patient_list,
	...
	fd = open('lr_tuning_results_' + scenario + '_' + case + '.json', 'w')
...
def best_parameters_architecture(hyper_parameters, results):
	...
	for lr in learning_rates:
		rmse = results[str(lr)]['overall']['AVG']['RMSE']
		if rmse < best_rmse:
			best_rmse = rmse
			best_parameters = lr
				
	best_lr = best_parameters
	hyper_parameters['learning_rate'] = best_lr
...
if __name__ == '__main__':
	...
	fd = open('tuned_lr_hyper_parameters_' + args.scenario + '_' + args.case + '.json', 'w')
